<?php
$saida_site = "http://www.cedro.ifce.edu.br";//http://localhost/secitifcedro
?>
...
if(isset($_POST["capt"])) {
	...
	echo "<script>alert('As letras digitadas estão incorretas!!!');</script>";
		...
		if(isset($_POST["email"])) {
			...
			$busca1 = mysql_query($sql,$link1);
			if(mysql_num_rows($busca1)==1){
				$row1 = mysql_fetch_assoc($busca1);
				$email = $row1["email"];
				$senha = $row1["senha"];
				$nome = $row1["nome"];
				$dado=$email . "&" . $senha . "&" . $nome . "&2";
				$cript=base64_encode($dado);
				$urls = $saida_site . "/sendmail_.php?c=" . $cript;
...
<div id="wrap">
    ...
    <div id="conteudo">
    <!-- InstanceBeginEditable name="Conteudo" --><h1>Recuperar a Senha</h1>
	<form id="form1" name="form1" method="post" action="recupera.php">
    <br />
    <h3>Digite seu e-mail de login para que seja enviada um mensagem contendo a senha.</h3>
    <br />
    <label>E-mail </label><input name="email" type="text" maxlength="40" /><br /><br />
    <label><img src="captcha/captcha.php" width="240" height="80" /></label><br />
    <label>Digite os Caracteres da imagem acima *</label><br />
    <input name="capt" type="text" maxlength="5" /><br />
    <input name="" type="submit" value="  Recuperar Senha  " />
    </form>
