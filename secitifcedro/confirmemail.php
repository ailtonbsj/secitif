<?php
$site_saida = "http://localhost/secitifcedro"; //http://www.cedro.ifce.edu.br
?>
...
<div id="wrap">
    ...
    <?php	
    if($row_Recordset1['ativo']=="T")
  {
    echo "<script>window.location.href=\"". "confirmpag.php?evt=" . $_GET["evt"] ."\"</script>";
	}
	else
	{
		if(isset($_GET["send"]))
	...
	?>
    <div style="text-align: center; font-family: Verdana, Geneva, sans-serif; margin: 20px;">Seu e-mail ainda não foi confirmado!<br />
    Para ter acesso ao sistema é necessário a confirmação de e-mail!<br /><br />
    <input type="button" name="button" id="button" value="Acessar Sistema sem confirmar!" style="background-color: #333;color: #FFF;font-size:18px;font-weight:bold;padding: 20px; cursor: pointer;" onclick="location.href='confirmpag.php?evt=<?= $_GET["evt"] ?>';" />
    <br /><br />
    <a href="confirmemail.php?send=ok">Enviar novamente o email de confirmação!</a>
    <br />
    <br />    
    Verifique sua caixa de Span caso não encontre o email na Caixa de Entrada.<br />
    Se não estiver conseguindo confirmar, por favor entre em contato!!! 
    </div>
	<?php
	}
	 ?>
